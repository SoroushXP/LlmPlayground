...
namespace LlmPlayground.Api.Tests.Services;
...
public class GameGeneratorServiceTests
{
    ...
    public GameGeneratorServiceTests()
    {
        ...
        _logger = Substitute.For<ILogger<GameGeneratorService>>();
    ...
    [Fact]
    public async Task GenerateGameAsync_WithCustomProvider_UsesSpecifiedProvider()
    {
        // Arrange
        var request = new GameGenerationRequest
        {
        ...
        _llmService.CurrentProvider.Returns("LmStudio");
        _llmService.SetProviderAsync(LlmProviderType.LmStudio, Arg.Any<CancellationToken>())
            .Returns(Task.CompletedTask);

        _llmService.ChatAsync(Arg.Any<ChatRequest>(), Arg.Any<CancellationToken>())
            .Returns(new CompletionResponse { Text = "Game idea", TokensGenerated = 10 });

        // Act
        var result = await _sut.GenerateGameAsync(request);

        // Assert
        result.ProviderUsed.Should().Be("LmStudio");
        await _llmService.Received(1)
            .SetProviderAsync(LlmProviderType.LmStudio, Arg.Any<CancellationToken>());
    }
    ...
    [Fact]
    public async Task GenerateGameAsync_WithUnsafePrologCode_SanitizesCode()
    {
        // Arrange
        var request = new GameGenerationRequest { ExecuteGame = false };
        ...
        _requestValidator.ValidatePrologQuery(Arg.Any<string>())
            .Returns(ValidationResult.Failure("Query", "Contains unsafe predicate: shell"));
        ...
        // Assert
        result.Success.Should().BeTrue();
        result.PrologCode.Should().Contain("% UNSAFE - commented out");
    }
